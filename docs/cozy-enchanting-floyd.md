...
- **Launcher OS banner.** `serve.py --osinfo` prints `System identified as <name> · <detail>`; the launcher calls it (after `chcp 65001` for the `·` on Windows) so the launch terminal shows e.g. `System identified as Windows 10 · 10.0.19045`.
- **Launchers renamed + split.** The `mission-control.cmd` sh/batch polyglot and the `mission-control.command` delegator became two clean per-OS files: **`mc-windows.cmd`** (pure batch) and **`mc-unix.command`** (pure sh, double-clickable). Windows autoclose now repeats the full "Autoclose in N.." on each line. Spinner verbs are WeZimplify's own fun set, rotating through a shuffled queue (no repeats, even across restarts). The offline hint names the right launcher per OS. All refs updated.
